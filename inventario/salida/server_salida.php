<?php
 ...
 'date_format(fecha_salida, "%d/%m/%Y") as fec'
	...
	$aColumnsCopiaSelect = array( 'id', '(select c.usuario from usuario c where c.id = t.usuario_id)', 'date_format(fecha_salida, "%d/%m/%Y")' );
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
		{
			...
			if ($i == 1 || $i == 2 )
			{
				...
				if ($i == 2)
				{
					$sWhere .= "date_format(fecha_salida, '%d/%m/%Y') LIKE '%".mysql_real_escape_string($_GET['sSearch_'.$i])."%' ";
				}
			}
			else
			{
