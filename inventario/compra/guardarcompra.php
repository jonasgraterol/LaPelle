...
if($_POST)
{
	...
	if(!validaLongitud($proveedorId, 0, 1, 5)) $error =1;
	if(!validaLongitud($fact, 0, 1, 50)) $error =2;
	if(!validaLongitud($fecha, 0, 8, 12)) $error =3;
	if(!validaLongitud($desc, 1, 3, 500)) $error =4;
	...
	{	
		...
		{ 
			$sql = "INSERT INTO ".$tabla." values(null,'".$id_usu."','".$descripcion."','".$fact."','".$proveedorId."',NOW(),'".fechaentrada($fecha)."',".$totalTotal.",1);";
			...
			if ($guardo_bien)
			{
				...
				for ($i = 0; $i < $n; $i++)
				{
					...
					$sql_detalle="INSERT INTO compra_detalle values('".$id_ali[$i]."','".$compra_id."','".$cant[$i]."','".$prec[$i]."','".$sub[$i]."');";
					...
					{
						$sql_cantidad = "UPDATE articulo set cantidad =".$nueva_cantidad.", fecha_ultima_compra= '".fechaentrada($fecha)."' where id =".$id_ali[$i];
			...
			if(!$mal_compra && !$mal_detalle && !$mal_compra)
			{
				echo "OK";
			}
			else
