<?php
	 ...
	 */
	$aColumns = array( 't.id', 't.imagen', 't.nombre', 't.precio1', 't.precio2', 't.precio3', 't.cantidad', 't.stock_minimo', 't.stock_maximo', 
  '(select c.nombre from tipoarticulo c where c.id = t.tipo_articulo_id) as nombreTipoArticulo',
  '(select cc.nombre from tipounidad cc where cc.id = t.tipo_unidad_id) as nombreTipoUnidad',
  'date_format(fecha_ultima_compra, "%d/%m/%Y") as fec',
   't.venta'
  );
	//ARREGLO GEMELO DE $aColumns PERO QUE CONTIENE LOS NOMBRES DE LOS CAMPOS EN LA BASE DE DATOS PARA PODER MOSTRAR LOS DATOS
	$aColumnsCopia = array( 'id', 'imagen', 'nombre' , 'precio1' , 'precio2' , 'precio3' , 'cantidad' , 'stock_minimo' , 'stock_maximo', 'nombreTipoArticulo', 'nombreTipoUnidad', 'fec', 'venta' );
	...
	$aColumnsCopiaSelect = array( 'id', 'imagen', 'nombre', 'precio1', 'precio2', 'precio3', 'cantidad', 'stock_minimo', 'stock_maximo', '(select c.nombre from tipoarticulo c where c.id = t.tipo_articulo_id)' , '(select cc.nombre from tipounidad cc where cc.id = t.tipo_unidad_id)' , 'date_format(fecha_ultima_compra, "%d/%m/%Y")', 'venta' );
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
		{
			...
			if ($i == 7 || $i == 8 || $i == 9 )
			{
				if ($i == 7)
				{
					$sWhere .= "(select c.nombre from tipoarticulo c where c.id = t.tipo_articulo_id) LIKE '%".mysql_real_escape_string($_GET['sSearch_'.$i])."%' ";
				}
				if ($i == 8)
				{
					$sWhere .= "(select cc.nombre from tipounidad cc where cc.id = t.tipo_unidad_id) LIKE '%".mysql_real_escape_string($_GET['sSearch_'.$i])."%' ";
				}
				if ($i == 9)
				{
