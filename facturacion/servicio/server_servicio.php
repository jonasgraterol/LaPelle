<?php
	 ...
	 */
	$aColumns = array( 't.id', 't.imagen', 't.nombre', 't.precio1', 
  '(select c.nombre from tiposervicio c where c.id = t.tipo_servicio_id) as nombreTipoServicio',
	't.descripcion',
  );
	//ARREGLO GEMELO DE $aColumns PERO QUE CONTIENE LOS NOMBRES DE LOS CAMPOS EN LA BASE DE DATOS PARA PODER MOSTRAR LOS DATOS
	$aColumnsCopia = array( 'id', 'imagen', 'nombre' , 'precio' , 'nombreTipoServicio', 'descripcion' );
	...
	$aColumnsCopiaSelect = array( 'id', 'imagen', 'nombre', 'precio', '(select c.nombre from tiposervicio c where c.id = t.tipo_servicio_id)', 'descripcion');
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
		{
			...
			if ($i == 4  )
			{
				if ($i == 4)
				{
					$sWhere .= "(select c.nombre from tiposervicio c where c.id = t.tipo_servicio_id) LIKE '%".mysql_real_escape_string($_GET['sSearch_'.$i])."%' ";
				}
