...
(function($){
    $.fn.smartCart = function(options) {
        ...
        return this.each(function() {
				...
				var attrProductUnidad = "punidad"; // Product Price attribute  
                var attrProductImage = "pimage"; // Product Image attribute
                var attrCategoryName = "pcategory";
                
                // Labels & Messages              
                var labelCartMenuName = 'Compra (_COUNT_)';  // _COUNT_ will be replaced with cart count
                var labelCartMenuNameTooltip = "Compra | Total Servicios: _PRODUCTCOUNT_ | Cantidad Totall: _ITEMCOUNT_";
                var labelProductMenuName = 'Servicios';
                var labelSearchButton = "Buscar";
                var labelSearchText = "";
                var labelCategoryText = "";
                var labelClearButton = "X";
                var labelAddToCartButton = "+"; 
                var labelQuantityText = "Cant.";
                var labelProducts = 'Servicios';
                var labelPrice = 'Precio';
                var labelSubtotal = 'Subtotal';
                var labelTotal = 'Total';
                var labelRemove = 'Quitar';
                var labelCheckout = 'Registrar';
                
                var messageConfirmRemove = 'Desea quitar "_PRODUCTNAME_" de la compra?'; //  _PRODUCTNAME_ will be replaced with actula product name
                var messageCartEmpty = "Compra vacia";
                var messageProductEmpty = "No hay Servicios para mostrar";
                var messageProductAddError = "El Servicio no pudo ser agreado";
                var messageItemAdded = 'Servicio agregado a la compra';
                var messageItemRemoved = 'Servicio eliminado de la compra';
                var messageQuantityUpdated = 'Cantidad de Servicio actualizada';
                var messageQuantityErrorAdd = 'Cantidad Invalida. El Servicio no pudo ser agreado';
                var messageQuantityErrorUpdate = 'Cantidad Invalida. No se pudo actualizar la cantidad';
                ...
                if(options.enableSearch){
                  var btSearch = $('<a>'+labelSearchButton+'</a>').attr("href","#").addClass("scSearch").attr("title","Buscar Servicio");                
                  $(btSearch).click(function() {
                     var searcStr = $(txtSearch).val();                      
                     populateProducts(searcStr);
                     return false;
                  }); 
                  var btClear = $('<a style="width:20px">'+labelClearButton+'</a>').attr("href","#").addClass("scSearch").attr("title","Limpiar Busqueda");
				...
				var elmBottomSubtotalValue2 = $('<input type="hidden" id="totalTotal" value="'+getMoneyFormatted(subTotal)+'" />');
                ...
                elmBottomBar.append(btCheckout).append(elmBottomSubtotalValue).append(elmBottomSubtotalValue2).append(elmBottomSubtotalText);
                ...
                function addToCart(i,qty){
					 ...
					 // SELECCIONAMOS EL ID DEL PRODUCTO PARA BUSCAR EL VALOR DE LA CAJA precio_'ide' Y ASIGNARLO A pprice PARA QUE PUEDA SER AGREGADO AL CARRO DE COMPRA (Agregado por jonas al 06-12-2012)
					 var ide = addProduct.attr("id").split("_");
					 var prc = $("#precio_"+ide[1]).val(); 
					 
					 $(addProduct).attr(attrProductPrice, prc)
					 if(addProduct.length > 0){
                        ...
                        if(productItem.length > 0){
                            ...
                            var lalQuantity =  $('#cantidad_'+pId).val(prdQty);
                            var lblTotal =  $('#lblTotal'+pId).html(prdTotal);
                ...
                function addCartItemDisplay(objProd,Quantity){
					...
					var comp = $('<input type="hidden" class="agre" value="id='+pId+' - cantidad='+Quantity+' - precio='+pPrice+' - subt='+pTotal+'" id="agre_'+pId+'" pid="'+pId+'" cantidad="'+Quantity+'" precio="'+pPrice+'" subt="'+pTotal+'" />');
                    var elmCPTitle2 = $('<label id="prec_'+pId+'">'+pPrice+'</label>').addClass("scCartItemTitle scCartItemTitle2");
                    var inputQty = $('<input type="text" value="'+Quantity+'" />').attr("id","cant_"+pId).attr("rel",pIndex).addClass("scTxtQuantity2");                    
                    ...
                    var elmCPTitle4 = $('<label>'+pTotal+'</label>').attr("id","lblTotal"+pId).addClass("scCartItemTitle scCartItemTitle4");
                    ...
                    elmCPTitle1.append(elmCPContent);
                    elmCP.append(comp).append(elmCPTitle1).append(elmCPTitle2).append(elmCPTitle3).append(elmCPTitle4).append(elmCPTitle5);
                ...
                function updateCartQuantity(idx,qty){
                    ...
                    var prdTotal = getMoneyFormatted(pPrice * qty);
                        // Now go for updating the design
                    var lblTotal =  $('#lblTotal'+prdId).html(prdTotal); 
                    showHighlightMessage(messageQuantityUpdated);
                    //Refresh the cart
                    refreshCartValues();
                ...
                function refreshCartValues(){
                    ...
                    elmBottomSubtotalValue.html(getMoneyFormatted(subTotal));
                  	elmBottomSubtotalValue2.val(getMoneyFormatted(subTotal));
                ...
                function populateProducts(searchString){
                   ...
                   $(products).each(function(i,n){
                      ...
                      if(isValid && isCategoryValid) {
                          ...
                          var inputQty = $('<input type="text" value="1" size="7"/>').addClass("scTxtQuantity");  
    ...
    $.fn.smartCart.defaults = {
          ...
          productItemTemplate:'<strong><%=pname%></strong> <br />Precio: <input onkeyup="comaXpunto(this)" class="prec" size="5" id="precio_<%=pid%>" />(<%=punidad%>)',
