...
(function($){
    $.fn.smartCart = function(options) {
        ...
        return this.each(function() {
				...
				var attrClase = "pclase";
                
                // Labels & Messages              
                var labelCartMenuName = 'Pedido (_COUNT_)';  // _COUNT_ will be replaced with cart count
                var labelCartMenuNameTooltip = "Pedido | Total Servicios: _PRODUCTCOUNT_ | Total Cantidad: _ITEMCOUNT_";
                var labelProductMenuName = 'Catalogo';
                var labelSearchButton = "Buscar";
                var labelSearchText = "Buscar";
                var labelCategoryText = "Categoria";
                var labelClearButton = "Limpiar";
                var labelAddToCartButton = "Agregar"; 
                var labelQuantityText = "Cantidad";
                var labelProducts = 'Servicios';
                var labelPrice = 'Precio';
                var labelSubtotal = 'Subtotal';
                var labelTotal = 'Total';
                var labelRemove = 'Quitar';
                var labelCheckout = 'Listo';
                
                var messageConfirmRemove = 'Desea quitar "_PRODUCTNAME_" del pedido?'; //  _PRODUCTNAME_ will be replaced with actula product name
                var messageCartEmpty = "Pedido Vacio";
                var messageProductEmpty = "No hay Servicios a mostrar";
                var messageProductAddError = "Servicio no pudo ser agregado";
                var messageItemAdded = 'Servicio agregado al pedido';
                var messageItemRemoved = 'Servicio removido del pedido';
                var messageQuantityUpdated = 'Cantidad del Servicio actualizada ';
                var messageQuantityErrorAdd = 'Cantidad invalida. Productto no agregado';
                var messageQuantityErrorUpdate = 'Cantidad invalida. Cantidad no actualizada';
                ...
                function addCartItemDisplay(objProd,Quantity){
                    ...
                    var inputQty = $('<input type="text" value="'+Quantity+'" />').attr("id","cantidad_"+pId).attr("rel",pIndex).addClass("scTxtQuantity2");                    
                ...
                function populateProducts(searchString){
                   ...
                   $(products).each(function(i,n){
                      ...
                      if(isValid && isCategoryValid) {
						  ...
						  //CLASES PARA TIPOS DE PRECIO (P1, P2 o P3)
                           var clase = $(this).attr(attrClase);
						  
                          var elmProdDiv1 = $('<div></div>').addClass("scPDiv1");
                          if(prodImgSrc && options.enableImage && prodImgSrc.length>0){
                              var prodImg = $("<img></img>").attr("src",prodImgSrc).addClass("scProductImage");   
							  var prodClase = $("<strong class='pp'></strong>").html(clase);
                              if(prodImg && options.enableImageTooltip){
                              	prodImg.bind("mouseenter mousemove",function(){
                                    showTooltip($(this));                    
                                  return false;
                              ...
                              elmProdDiv1.append(prodClase).append(prodImg);
                          }
                          ...
                          var btAddToCart = $('<a>'+productPrice+'</a>').attr("href","#").attr("rel",i).attr("title",productPrice).addClass("scAddToCart");
                          ...
                          elmProdDiv3.append(inputQty).append(btAddToCart);                  
    						//alert(clase);
                          var elmProds = $('<div class="'+clase+'"></div>').addClass("scProducts");
                ...
                function showTooltip(img) {
                  ...
                  jQuery.log("--------------");
                  tooltip.html('');
                  var tImage = $("<img width='200' height='150'></img>").attr('src',$(img).attr('src')); 
                  //tImage.height(toolMaxImageHeight);
                  tooltip.append(tImage);
              		var top = imgOffsetTop - 300;
              		var left = imgOffsetLeft -385;
    ...
    $.fn.smartCart.defaults = {
          ...
          productItemTemplate:'<strong><%=pname%></strong>',
