...
function limpiaFormTipoServicio()
{
	for(i=0; i<=2; i++)
	{
...
function blanquearTipoServicio()
{
	for(i=0; i<=2; i++)
	{
...
function validaTipoPla(modulo,accion,id)
{
	
	cargarDatos(); 
	//limpiaFormTipoServicio();
	escondeError();
	error=0;
	...
	else
	{
		...
		ajax.send("accion="+accion+"&id="+id+"&nombre="+nomb+"&desc="+desc);
				...
				{	
					...
					else
					{
						...
						if(ok!=-1)
						{
							...
							//setTimeout("cargarDiv('left','modulos/renglon/crearTipoServicio.php');",4000);
