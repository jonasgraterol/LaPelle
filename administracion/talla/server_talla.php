<?php
	 ...
	 */
	$aColumns = array( 't.id', '(select c.nombre from tipoarticulo c where c.id = t.tipo_articulo_id) as nombreTipoArticulo', 't.nombre');
	$aColumnsCopia = array('id', 'nombreTipoArticulo', 'nombre');
	$aColumnsCopiaSelect = array('id', '(select c.nombre from tipoarticulo c where c.id = t.tipo_articulo_id)', 'nombre');
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			if ( $aColumns[$i] == "version" )
			{
				...
				$row[] = ($aRow[ $aColumns[$i] ]=="0") ? '-' : $aRow[ $aColumnsCopia[$i] ];
			}
			else if ( $aColumns[$i] != ' ' )
			{
				/* General output */
