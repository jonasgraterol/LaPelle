...
function validaTipoUni(modulo,accion,id)
{
	
	cargarDatos(); 
	//limpiaFormTipoUnidad();
	escondeError();
	error=0;
	...
	if(!validaLongitud(abrev, 1, 1, 5)) muestraError(form.abrev.id);
	...
	else
	{
		...
		ajax.send("accion="+accion+"&id="+id+"&nombre="+nomb+"&abrev="+abrev);
				...
				{	
					...
					else
					{
						...
						if(ok!=-1)
						{
							...
							//setTimeout("cargarDiv('left','modulos/renglon/crearTipoUnidad.php');",4000);
