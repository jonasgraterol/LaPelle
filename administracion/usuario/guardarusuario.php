...
if($_POST)
{
	...
	if(!validaLongitud($clave, 0, 1, 20)) $error = 2;
	
	
	$tabla="usuario";
	$nombreViejo = buscar_campo_segun_campo_tabla_status("id",$id,$tabla,"usuario",1);
	...
	{	
		...
		else
		{
			...
			{ 
				$sql="INSERT INTO ".$tabla." values(null,'".$nombre."','".$clave."',1);";
	...
	if ($accion == "editar")
	{
		...
		else
		{
			...
			{
				//$id= $_GET['id'];
				$sql="UPDATE ".$tabla." SET usuario = '".$nombre."', clave = '".$clave."' WHERE id = '".$id."'";
